...
class OnboardingWizard {
    constructor(context) {
        ...
        this.metadataStoragePath = '';
        this.configStoragePath = '';
        this.isCapturing = false;
        this.currentSession = null;
        this.filesTreeProvider = null;
    ...
    async switchProject() {
        ...
        const action = await vscode.window.showInformationMessage(message, 'Create New Project', 'Cancel');
        ...
        try {
            ...
            for (const key of keys) {
                if (key.startsWith('qa-html-capture.project.') && key.endsWith('.details')) {
                    const projectDetails = workspaceState.get(key);
                    if (projectDetails) {
    ...
    async switchToProject(projectName) {
        try {
            ...
            const projectDetails = this.context.workspaceState.get(`${projectKey}.details`);
            if (!projectDetails) {
    ...
    async saveProjectConfigToFile(projectDetails) {
        try {
    ...
    async showCompletionMessage(details) {
        const environmentsText = details.environments.map(env => `• ${env.name}: ${env.url}${env.isDefault ? ' (Default)' : ''}`).join('\n');
...
Would you like to reconfigure these settings?`;
    ...
    async uploadManualTestCasesFile() {
    ...
    async handleManualFlowCapture(projectDetails) {
...
Note: Closing the browser will automatically stop capture and save your metadata.`;
        ...
        const stopCapture = await vscode.window.showInformationMessage(instructions, 'Stop Capture Now', 'Cancel');
    ...
    async handleTestGeneration(projectDetails) {
    ...
    async generateTestsFromMetadata(projectDetails) {
    ...
    async generateTestsFromFile(projectDetails) {
    ...
    async generateTestsFromPageNames(projectDetails) {
    ...
    async showGeneratedTestsForReview(projectDetails) {
...
console.log('Description: ${description}');`;
    ...
    async saveCapturedMetadata(metadata) {
        try {
    ...
    async addManualInteraction(interaction) {
        if (this.currentSession && this.isCapturing) {
            this.addInteraction({
                ...interaction,
                timestamp: new Date().toISOString()
    ...
    async openProjectConfigFile() {
        try {
            const configFilePath = path.join(this.configStoragePath, 'project-config.json');
            // Check if config file exists
            if (!fs.existsSync(configFilePath)) {
                const createFile = await vscode.window.showWarningMessage('Project configuration file not found. Would you like to create one?', 'Create File', 'Cancel');
            ...
            const copyPath = await vscode.window.showInformationMessage(`Config file location: ${configFilePath}`, 'Copy Path', 'OK');
    ...
    async reloadProjectConfigFromFile() {
        try {
            const configFilePath = path.join(this.configStoragePath, 'project-config.json');
            if (!fs.existsSync(configFilePath)) {
    ...
    async openBrowserInEnvironment(url, environment) {
        try {
            switch (environment) {
                case 'default':
    ...
    async openNewBrowserWindow(url) {
        ...
        if (isWSL) {
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    this.browserProcess = exec(command, (error, stdout, stderr) => {
                        if (error) {
    ...
    async openIncognitoBrowser(url) {
        const { spawn, exec } = require('child_process');
        // Check if we're in WSL environment
        const isWSL = process.env.WSL_DISTRO_NAME || process.env.WSLENV;
        if (isWSL) {
...
Interactions:
${metadata.interactions.map((interaction, index) => `${index + 1}. ${interaction.action}${interaction.element ? ` on ${interaction.element}` : ''}${interaction.value ? ` with value "${interaction.value}"` : ''} at ${interaction.timestamp}`).join('\n')}`;
